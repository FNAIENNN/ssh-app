...
with open('/home/user/.gemini/antigravity-ide/brain/c69681dd-917b-4697-91d4-3aeba311abe4/.system_generated/logs/transcript_full.jsonl', 'r') as f:
    for line in f:
        try:
            obj = json.loads(line)
            if obj.get('type') == 'VIEW_FILE' and obj.get('content') and 'SeedStocking.jsx' in obj['content'] and 'Packing' in obj['content']:
                count += 1
                ...
                with open(f'SeedStocking_snippet_{count}.jsx', 'w') as out:
                    out.write('\n'.join(actual_lines))
