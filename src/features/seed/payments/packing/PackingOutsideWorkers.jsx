...
import SignaturePad from '../seedStocking/SignaturePad';
...
export default function PackingOutsideWorkers({ 
  ...
  async function handleSaveData() {
    ...
    if (!supervisorName.trim()) return toast.error('Enter Mestri Name');
    if (!supervisorSignature) return toast.error('Provide Mestri Sign Off');
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="card p-6 space-y-6 shadow-sm border" style={{ borderColor: 'var(--color-primary)' }}>
        ...
        <h4 className="font-extrabold text-lg text-primary border-b pb-2">2. Worker Payments</h4>
        ...
        {/* Mestri Details & Digital Signature */}
        <div className="card p-5 space-y-4 border bg-slate-50 mt-6">
          <h4 className="font-extrabold text-base text-primary border-b pb-2">✍️ 3. Mestri Sign Off</h4>
          ...
          <div>
            <label className="field-label">Mestri Sign Off *</label>
        ...
        >
          {submitting ? '⏳ Processing...' : `✅ Complete Packing`}
        </button>
      </div>
    </div>
  );
