...
export default function PackingDetails({ tanks, setTanks, vehicles = [], onNext }) {
  ...
  const step1Tanks = tanks.filter(t => !t.isTransferTarget);
  ...
  const handlePacketsChange = (id, val) => {
    ...
    if (val === '' || /^\d+$/.test(val)) {
      setTanks(prev => prev.map(t => {
        if (t.id === id) {
          const prevPackets = Number(t.originalPackets ?? t.numberOfPackets) || 0;
          const newPackets = Number(val) || 0;
          const diff = newPackets - prevPackets;
          
          return { 
            ...t, 
            originalPackets: val,
            numberOfPackets: (Number(t.numberOfPackets) || 0) + diff
          };
        }
        return t;
      }));
  ...
  const handleSaveVehicle = () => {
    ...
    const vTanks = step1Tanks.filter(t => tids.includes(t.id));
    const missing = vTanks.some(t => {
      const pkts = t.originalPackets ?? t.numberOfPackets;
      return !pkts || Number(pkts) <= 0;
    });
    if (missing) {
      alert("Please enter a valid number of packets for all tanks assigned to this vehicle.");
      return;
    }
  ...
  const unassignedTanks = step1Tanks.filter(t => !assignedTankIds.has(t.id));
          ...
          {tankList.map((t) => (
            <tr key={t.id} className="border-t hover:bg-slate-50 transition" style={{ borderColor: 'var(--color-border)' }}>
              ...
              <td className="p-3 font-extrabold text-primary border-r" style={{ borderColor: 'var(--color-border)' }}>
                {Number(t.originalQuantity ?? t.quantity).toLocaleString('en-IN')}
              </td>
              <td className="p-3">
                <input
                  type="text"
                  ...
                  value={t.originalPackets ?? t.numberOfPackets}
