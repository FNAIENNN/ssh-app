...
/**
 * HarvestDashboard — Real-time command center.
 * Replaced Executive Command Center banner with FCR Calculation Table.
 * FCR = Total Feed / Total Tonnage
 */
export default function HarvestDashboard({ siteId, onStartMiddleHarvest, onStartFullHarvest }) {
  ...
  // FCR table state — single row for search/calculation
  const [fcrFeed, setFcrFeed] = useState('');
  const [fcrTonnage, setFcrTonnage] = useState('');
  ...
  // FCR Calculation
  const fcrFeedNum = Number(fcrFeed) || 0;
  const fcrTonnageNum = Number(fcrTonnage) || 0;
  const fcrResult = fcrTonnageNum > 0 ? (fcrFeedNum / fcrTonnageNum).toFixed(3) : '—';
  ...
  return (
    <div className="space-y-6 pb-6">
      {/* FCR Calculation Table — Replaces Executive Harvest Command Center */}
      <div className="rounded-2xl p-5 bg-white border border-slate-200 shadow-card space-y-4">
        <div>
          <h2 className="text-base font-extrabold text-slate-900 flex items-center gap-2">
            📊 FCR Calculation Table
          </h2>
          <p className="text-xs text-slate-500 mt-0.5">
            Feed Conversion Ratio — FCR = Total Feed (KG) ÷ Total Tonnage (KG). Enter values below to calculate.
          </p>
        </div>
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-slate-900 text-white font-extrabold">
                <th className="p-3 w-14 text-center text-[11px]">S.No</th>
                <th className="p-3 text-[11px]">Total Feed (KG)</th>
                <th className="p-3 text-[11px]">Total Tonnage (KG)</th>
                <th className="p-3 text-right text-[11px]">FCR Report</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {/* Single search/calculation row */}
              <tr className="bg-blue-50/40 hover:bg-blue-50 transition">
                <td className="p-3 text-center font-bold text-slate-500">1</td>
                <td className="p-3">
                  <input
                    type="number"
                    step="0.1"
                    placeholder="Enter total feed in kg"
                    value={fcrFeed}
                    onChange={(e) => setFcrFeed(e.target.value)}
                    className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 font-mono text-sm font-bold text-slate-900 focus:border-blue-500 focus:outline-none"
                  />
                </td>
                <td className="p-3">
                  <input
                    type="number"
                    step="0.1"
                    placeholder="Enter total tonnage in kg"
                    value={fcrTonnage}
                    onChange={(e) => setFcrTonnage(e.target.value)}
                    className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 font-mono text-sm font-bold text-slate-900 focus:border-blue-500 focus:outline-none"
                  />
                </td>
                <td className="p-3 text-right">
                  <div className={`inline-flex flex-col items-end rounded-xl px-4 py-2 ${fcrTonnageNum > 0 ? 'bg-emerald-100 border border-emerald-300' : 'bg-slate-100 border border-slate-200'}`}>
                    <span className="text-[10px] font-bold text-slate-500 uppercase">FCR</span>
                    <span className={`text-lg font-black font-mono ${fcrTonnageNum > 0 ? 'text-emerald-800' : 'text-slate-400'}`}>
                      {fcrResult}
                    </span>
                  </div>
                </td>
        ...
        <div className="flex gap-3 flex-wrap">
          {[
            { range: '< 1.2', label: 'Excellent', color: 'text-emerald-700 bg-emerald-50 border-emerald-200' },
            { range: '1.2 – 1.5', label: 'Good', color: 'text-blue-700 bg-blue-50 border-blue-200' },
            { range: '1.5 – 1.8', label: 'Average', color: 'text-amber-700 bg-amber-50 border-amber-200' },
            { range: '> 1.8', label: 'Poor', color: 'text-red-700 bg-red-50 border-red-200' },
          ].map((guide) => (
            <span key={guide.range} className={`text-[10px] font-bold px-2.5 py-1 rounded-full border ${guide.color}`}>
              FCR {guide.range} → {guide.label}
            </span>
          ))}
        </div>
      </div>
