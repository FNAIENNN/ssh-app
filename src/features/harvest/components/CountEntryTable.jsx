...
/**
 * CountEntryTable — Step 5: Count sample entry & Price per Kg calculation.
 * Count = Pieces ÷ Kgs, shown with decimal precision (e.g. 33.33 for 100 pcs / 3 kg).
 */
export default function CountEntryTable({
  ...
  // Compute selected count value — with decimal precision
  ...
  // Decimal count: do NOT round — show 2 decimal places
  const finalCalculatedCount = selectedKg > 0
    ? parseFloat((selectedPcs / selectedKg).toFixed(2))
    : 0;
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Count = Pieces ÷ Kgs (shown with decimals). Select one row as the Final Harvest Count.
            </p>
          </div>
          <button
            type="button"
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 font-medium text-slate-800">
              {countRows.map((row, idx) => {
                ...
                // Decimal count with 2 decimal places
                const calcCount = kgs > 0 ? parseFloat((pcs / kgs).toFixed(2)) : 0;
                  ...
                  >
                    ...
                    <td className="p-3">
                      <input
                        ...
                        className="w-full bg-slate-50 border border-slate-300 rounded-lg px-3 py-1.5 font-mono text-sm text-slate-900 focus:bg-white focus:border-blue-500 focus:outline-none"
                      />
                    </td>
                    <td className="p-3">
                      <input
                    ...
                    <td className="p-3 text-right font-extrabold font-mono text-blue-700 text-sm">
                      {calcCount > 0 ? (
                        <span>
                          {calcCount.toFixed(2)}{' '}
                          <span className="text-[10px] font-normal text-slate-400">count/kg</span>
                        </span>
                      ) : '—'}
                    </td>
                    <td className="p-3 text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
          ...
          <div className="rounded-xl p-4 bg-gradient-to-br from-blue-900 to-slate-900 text-white shadow-md">
            ...
            <div className="flex items-baseline gap-1 mt-1">
              <span className="text-3xl font-black font-mono text-white">{finalCalculatedCount.toFixed(2)}</span>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                onWheel={preventWheel}
                className="w-full bg-slate-50 border border-slate-300 rounded-xl pl-8 pr-3 py-2.5 text-lg font-black font-mono text-slate-900 focus:bg-white focus:border-blue-600 focus:outline-none"
              />
            </div>
            <span className="text-[10px] text-slate-500 block mt-1">
              Agreed buyer rate per kg for {finalCalculatedCount.toFixed(2)} count
            </span>
          </div>
