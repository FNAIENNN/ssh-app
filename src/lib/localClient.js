...
const STORAGE_KEY = 'ssh.demoDb.v5';
...
function loadDb() {
  try {
    localStorage.removeItem('ssh.demoDb.v2');
    localStorage.removeItem('ssh.demoDb.v1');
    localStorage.removeItem('ssh.demoDb');
...
export function resetDemoData() {
  ...
  localStorage.removeItem('ssh.demoDb.v3');
  localStorage.removeItem('ssh.demoDb.v2');
  localStorage.removeItem('ssh.demoDb.v1');
  localStorage.removeItem('ssh.demoDb');
  return loadDb();
  ...
  return rows.filter((r) =>
    filters.every((f) => {
      ...
      switch (f.op) {
        ...
        case 'like':
        case 'ilike': return typeof v === 'string' && v.toLowerCase().includes(String(f.value).replace(/%/g, '').toLowerCase());
        default: return true;
      }
    })
  );
